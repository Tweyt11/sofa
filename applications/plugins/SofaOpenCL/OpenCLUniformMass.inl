...
void UniformMass<OpenCLVec3Types, double>::addMDx(const core::MechanicalParams* /*mparams*/ /* PARAMS FIRST */, DataVecDeriv& d_f, const DataVecDeriv& d_dx, SReal d_factor)
{
    ...
    UniformMassOpenCL3d_addMDx(dx.size(), (double)(d_totalMass.getValue()*d_factor), f.deviceWrite(), dx.deviceRead());
...
void UniformMass<OpenCLVec3Types, double>::accFromF(const core::MechanicalParams* /*mparams*/ /* PARAMS FIRST */, DataVecDeriv& d_a, const DataVecDeriv& d_f)
{
    ...
    UniformMassOpenCL3d_accFromF(f.size(), d_totalMass.getValue(), a.deviceWrite(), f.deviceRead());
...
void UniformMass<OpenCLVec3Types, double>::addForce(const core::MechanicalParams* /*mparams*/ /* PARAMS FIRST */, DataVecDeriv& d_f, const DataVecCoord& /*d_x*/, const DataVecDeriv& /*d_v*/)
{
    ...
    Deriv mg = theGravity * d_totalMass.getValue();
...
void UniformMass<OpenCLVec3d1Types, double>::addMDx(const core::MechanicalParams* /*mparams*/ /* PARAMS FIRST */, DataVecDeriv& d_f, const DataVecDeriv& d_dx, SReal d_factor)
{
    ...
    UniformMassOpenCL3d1_addMDx(dx.size(), (double)(d_totalMass.getValue()*d_factor), f.deviceWrite(), dx.deviceRead());
...
void UniformMass<OpenCLVec3d1Types, double>::accFromF(const core::MechanicalParams* /*mparams*/ /* PARAMS FIRST */, DataVecDeriv& d_a, const DataVecDeriv& d_f)
{
    ...
    UniformMassOpenCL3d1_accFromF(f.size(), d_totalMass.getValue(), a.deviceWrite(), f.deviceRead());
...
SReal UniformMass<gpu::opencl::OpenCLRigid3Types,sofa::defaulttype::Rigid3Mass>::getPotentialEnergy(const core::MechanicalParams* /*mparams*/ /* PARAMS FIRST */, const DataVecCoord& d_x) const
{
    ...
    for (unsigned int i=0; i<x.size(); i++)
    {
        e += g*d_totalMass.getValue().mass*x[i].getCenter();
    }
    return e;
}
...
{
    return (SReal)(d_totalMass.getValue().mass);
}
...
//	if (!getContext()->getShowBehaviorModels())return;
    ...
    double m00 = d_totalMass.getValue().inertiaMatrix[0][0];
    double m11 = d_totalMass.getValue().inertiaMatrix[1][1];
    double m22 = d_totalMass.getValue().inertiaMatrix[2][2];
