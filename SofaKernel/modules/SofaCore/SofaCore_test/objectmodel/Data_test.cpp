...
{
    EXPECT_FALSE(dataInt1.getDDGNode()->isDirty());
    dataInt1.setValue(1);
    EXPECT_FALSE(dataInt1.getDDGNode()->isDirty());
}
...
TEST_F(Data_test, setParentWithInit)
{
    ...
    dataInt2.setValue(0);
    dataInt2.setParent(&dataInt1);
    ASSERT_FALSE(dataInt1.getDDGNode()->isDirty());
    ASSERT_TRUE(dataInt2.getDDGNode()->isDirty());

    ASSERT_EQ(dataInt1.getValue(), 1);
    ASSERT_EQ(dataInt2.getValue(), 1);

    ASSERT_FALSE(dataInt1.getDDGNode()->isDirty());
    ASSERT_FALSE(dataInt2.getDDGNode()->isDirty());
}
